...
void * receive_info (void *arg)
{
	...
	while(1) {
		recvfrom(sock, received_packet, sizeof(received_packet), 0, (struct sockaddr *)&otheraddr, &addr_size);

		int sender_id = ntohl(received_packet[0]);
		int neighbor_id = ntohl(received_packet[1]);
		...
		printf("New node: %d: %d -> %d, new cost = %d\n", sender_id, sender_id, neighbor_id, new_cost);
		print_costs();
	}
